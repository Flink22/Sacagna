...
void init_pid(){ //Abilito t/c 0 per fare pid ogni intervallo di tempo (100 Hz)
...
ISR(TIMER0_COMPA_vect){
	...
	speed = 0;
	speed = 1000000/(mot_curr[pid_mot] * 990) * mot_dir[pid_mot];
	...
	speed_error = wanted_speed - speed;
	integral[pid_mot] += speed_error * 0.039936;
	derivative = (speed_error - old_error[pid_mot]) / 0.039936;
	old_error[pid_mot] = speed_error;

	correction = (speed_error * kp) + (derivative * kd) + (integral[pid_mot] * ki);
	if(correction > 1023)
	correction = 1023;
	if(correction < 1023)
	correction = -1023;
